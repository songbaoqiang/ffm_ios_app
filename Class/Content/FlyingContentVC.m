...
@property (strong, nonatomic) FlyingGameView     *gameView;
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    if(![NSString isBlankString:self.thePubLesson.subtitleURL])
    {
        // Show coach marks
        BOOL coachMarksShown = [[NSUserDefaults standardUserDefaults] boolForKey:@"FlyingContentVCMarksShown"];
        
        if (coachMarksShown == NO) {
            // Don't show again
            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"FlyingContentVCMarksShown"];
            [[NSUserDefaults standardUserDefaults] synchronize];
            
            // Setup coach marks
            NSArray *coachMarks;
            
#ifdef __DOMAIN_ENGLISH__
            if(INTERFACE_IS_PHONE)
            {
               coachMarks  = @[
                                        @{
                                            @"rect": [NSValue valueWithCGRect:self.mediaVC.view.frame],
                                            @"caption": @"技巧:点击字幕取词",
                                            @"shape": @"square"
                                            },
                                        @{
                                            @"rect": [NSValue valueWithCGRect:CGRectOffset(self.authoTablecell.frame, self.tableView.frame.origin.x, self.tableView.frame.origin.y)],
                                            @"caption": @"技巧：咨询作者和客服",
                                            @"shape": @"square"
                                            },
                                        ];
            ...
            else
            {
                coachMarks = @[
                                        ...
                                        @{
                                            @"rect": [NSValue valueWithCGRect:CGRectOffset(self.loadingMoreIndicatorCell.frame, self.tableView.frame.origin.x, self.tableView.frame.origin.y)],
                                            @"caption": @"技巧：这里添加评论:)",
                                            @"shape": @"square"
                                            },
                                        ];
...
- (void) commonInit
{
    ...
    if (!self.tableView)
    {
        self.tableView = [[UITableView alloc] initWithFrame: CGRectMake(0.0f, CGRectGetHeight(self.coverContentView.frame), CGRectGetWidth(self.view.frame),CGRectGetHeight(self.view.frame)-CGRectGetHeight(self.coverContentView.frame)-64) style:UITableViewStyleGrouped];
...
- (void) playLesson:(NSString *) lessonID
{
    if([self.thePubLesson.contentType isEqualToString:KContentTypeVideo]||
       [self.thePubLesson.contentType isEqualToString:KContentTypeAudio])
    {
        [self playVedio];
    }
    else if ([self.thePubLesson.contentType isEqualToString:KContentTypeText])
    {
        ...
        if ([extention isEqualToString:@"pdf"] ||
            [extention isEqualToString:@"epub"] )
        {
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.section == 1)
    {
        switch (indexPath.row)
        {
            ...
            case 1:
            {
                if (![NSString isBlankString:self.thePubLesson.desc]) {
...
//////////////////////////////////////////////////////////////
#pragma FlyingMediaVCDelegate related
//////////////////////////////////////////////////////////////
...
- (void)playGameForSub:(FlyingSubTitle*)subtitle GameIndex:(NSInteger) gameIndex;
{
    ...
    if(!self.gameView)
    {
        ...
        gameViewFrame.size.width  = frame.size.width*3/4;
        gameViewFrame.size.height = height*4;
        gameViewFrame.origin.x    = frame.size.width/8;
        gameViewFrame.origin.y    = CGRectGetHeight(self.tableView.frame)-height;
        
        self.gameView = [FlyingGameView gameView];
    ...
    [self.gameView loadSubtitle:subtitle];
    [self.gameView beginGameAtIndex:gameIndex];
    
    [self.view bringSubviewToFront:self.gameView];
}

//////////////////////////////////////////////////////////////
#pragma FlyingGameViewDelegate related
//////////////////////////////////////////////////////////////

- (void)setMagicGameAt:(NSInteger) index;
{
    [self.mediaVC setMagicGameAt:index];
}
